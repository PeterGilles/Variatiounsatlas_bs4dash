...
#color_palette <- c('#7fc97f','#beaed4','#fdc086','#ffff99','#386cb0','#f0027f','#bf5b17','#666666')
...
ui <- function(request) {
  ...
  titlePanel(title=div(id="dummy", img(src = "logo.png")), "Variatiounsatlas vum Lëtzebuergeschen"),
    sidebarLayout(
      sidebarPanel(
        width = 2,
        selectInput(
        ...
        selectInput("input_choice", "Kaart", choices = NULL),
        #bookmarkButton()
      ),
      mainPanel(
        width = 10,
        fluidRow(
          tags$h3("Iwwerbléckskaart"),
          plotOutput("Iwwerbléckskaart", height = "800px")),
        fluidRow(tags$h3("Variantekaarten"),
                 plotOutput("Variantekaarten", height = "800px")),
        tags$h3("Korrelatioun mat Sozialdaten"),
        tabsetPanel(
          type = "tabs",
          ...
          tabPanel("Audio", plotOutput("plotAudio")),
          fluidRow(
            tags$h3("Conditional Inference Tree"),
...
server <- function(input, output, session) {
  ...
  input_choice <- reactive({
    ...
    output$Iwwerbléckskaart <- renderPlot({
      # prepare data
      #variableFonInput <- as.character(input_choice$variable)
    ...
    output$Variantekaarten <- renderPlot({
      ...
      bindCache(input_choice()$variable)
    ...
    observeEvent(map_category(), {
      updateSelectInput(session, "input_choice", choices = unique(map_category()$input_choice), selected = "Fra")
    })
